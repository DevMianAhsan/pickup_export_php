						...
						<div class="col-sm-6">
							...
							<select class="form-control select2" id="inquiryvehicle" name="inquiryvehicle">
								...
								while ($r = mysqli_fetch_assoc($q)):
									...
									?>
									<option value="<?= $r['vehicle_id'] ?>"><?= $maker['maker_name'] ?> 		<?= $brand['brand_name'] ?>
									</option>
								<?php endwhile ?>
							</select>
...
<?php if ($_REQUEST['o'] == 'list'): ?>
	...
	<div class="col-sm-12">
		<div class="card">
			<div class="card-header card-bg" align="center"> <b class="h4 text-center card-text">Inquiry Lists
					<?= $_REQUEST['inquiry_of'] ?></b></div>
			<div class="card-body">
				<div class="table-responsive">
					<table class="table dataTable" id="formDataIQTb">
						<thead>
							<?php $inquiryType = strtolower($_GET['inquiry_of'] ?? ''); ?>
							<?php if ($inquiryType === 'contact'): ?>
								<tr>
									<th>Inquiry ID</th>
									<th>Name</th>
									<th>Email</th>
						...
						<tbody>
							...
							while ($r = mysqli_fetch_assoc($q)):
								$servicesLabel = '';
								if ($inquiryType === 'contact' && !empty($r['inquiry_services'])) {
									$decodedServices = json_decode($r['inquiry_services'], true);
									if (is_array($decodedServices)) {
										$servicesLabel = implode(', ', array_filter($decodedServices));
									} else {
										$servicesLabel = $r['inquiry_services'];
									}
								}
...
<?php if ($inquiryType === 'contact'): ?>
									<tr>
										...
										<td><?= htmlspecialchars($r['inquiry_name']) ?></td>
										<td><a href="mailto:<?= htmlspecialchars($r['inquiry_email']) ?>"><?= htmlspecialchars($r['inquiry_email']) ?></a></td>
										<td><a href="tel:<?= htmlspecialchars($r['inquiry_phone']) ?>"><?= htmlspecialchars($r['inquiry_phone']) ?></a></td>
										<td><?= nl2br(htmlspecialchars($r['inquiry_msg'])) ?></td>
										<td><?= htmlspecialchars($servicesLabel) ?></td>
									...
									<tr>
										...
										<td><?= @$maker['maker_name'] ?> 		<?= @$brand['brand_name'] ?><br />
											<?php if ($_GET['inquiry_of'] == 'vehicle') {
												?>
												<button onclick="loadData('vehicle_info', <?= $vehicle['vehicle_id'] ?>)"
													class="dropdown-item text-success view">Vehicle Info</button>
												<?php
											} elseif ($_GET['inquiry_of'] == 'part') {
												?>
												<button onclick="loadData('vehicle_parts', <?= $vehicle['part_id'] ?>)"
													class="dropdown-item text-success view">Part Info</button>
												<?php
											} elseif ($_GET['inquiry_of'] == 'machine') {
												?>
												<button onclick="loadData('machines', <?= $vehicle['machine_id'] ?>)"
													class="dropdown-item text-success view">Machine Info</button>
												<?php
											} else {
